	     ...
	     <div class="col-lg-3 mb-4">
	        <div class="list-group">
		        <c:forEach var="cityName_L" items="${cityName}">
		        	<c:if test="${cityName_L.catLv eq 1}">
						<li class="list-group-item" id="${cityName_L.catCd}"><c:out value="${cityName_L.catNm}"/></li>
					</c:if>
				</c:forEach>
	        </div>
	      </div>
	      	...
	      	<c:if test="${cityName_L.catLv eq 1}">
			      <div class="col-lg-9 mb-4 subMenu" style="display: none">
		    		<c:forEach var="cityName_M" items="${cityName}">
			      		<c:if test="${(cityName_M.catLv eq 2) and (cityName_L.catCd eq cityName_M.upprCat)}">
			      				<li class="list-group-item">
			      					<a href=<c:url value="${cityName_M.cnntUrl}"/>><c:out value="${cityName_M.catNm}"/></a>
			      				</li>
			      		</c:if>
		    		</c:forEach>
			     </div>
